...
data <- lapply(
  ...
  filter(
    post_date >= as_date("2025-09-19") & post_date <= as_date("2025-12-31")
  ) %>% 
  select(
...
# 安踏股价。
anta_stock <-
  tidyquant::tq_get("2020.HK", from = "2025-09-05", to = "2025-12-31")
...
## Trend fig ----
time_fig_x_date <- c(
  "2025-9-19", "2025-9-26", "2025-10-01", "2025-10-15",
  "2025-11-01", "2025-11-15", "2025-12-01", "2025-12-31"
)
...
(
  ggplot(all_dt_num_smry, aes(post_date)) +
    ...
    scale_x_date(
      limits = c(as.Date("2025-09-18"), as.Date("2026-01-02")),
  ...
  (
    ...
    ggplot(all_dt_num_smry) + 
      ...
      scale_fill_manual(
        values = c("TRUE" = "orange", "FALSE" = "darkgreen"),  # 大于0红色，否则绿色
        guide = "none"  # 不显示图例
      ) +
      scale_x_date(
        limits = c(as.Date("2025-09-18"), as.Date("2026-01-02")),
...
get_cor_circle_styled <- function(df_x, label_map = NULL, 
  ...
  merged <- merged %>%
    mutate(
      ...
      abs_cor = abs(cor),
      # 相关方向（正/负）
      cor_dir = ifelse(cor >= 0, "Positive", "Negative"),
      cor_dir = factor(cor_dir, levels = c("Positive", "Negative")),
      # 显著性标记
      p_mark = case_when(
        p < 0.001 ~ "***",
        p < 0.01  ~ "**",
  ...
  p <- ggplot(merged, aes(x = Var1_label, y = Var2_label)) +
    ...
    geom_point(aes(size = abs_cor, color = cor_dir), alpha = 1) +
    # 为不显著的点添加白色密集点覆盖层
    geom_point(
      data = non_sig_data,
      aes(x = x, y = y),
    ...
    # 设置圆圈颜色：橙红色(正相关)，蓝色(负相关)
    scale_color_manual(
      values = c("Positive" = "#FC9272", "Negative" = "#6BAED6"),
...
dfmat_tag <- weibo_corpus %>%
  ...
  dfm_select(., pattern = "#*")
# 提取前 20 个高频标签
top_tag <- names(topfeatures(dfmat_tag, 20))
# 各日期各标签的频度。
df_date_freq <-
  # 筛选 DFM，只保留这 20 个标签
